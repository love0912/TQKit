...
@implementation UIImage (TTExt)

/**
 *   纯色图片
 */
+ (UIImage *)tt_imageWithColor:(UIColor *)color {
    return [self tt_imageWithColor:color size:CGSizeMake(1, 1)];
}
...
+ (NSData *)tt_compressImage:(UIImage *)image toMaxLength:(NSInteger) maxLength maxWidth:(NSInteger)maxWidth {
    ...
    UIImage *newImage = [image tt_imageByResizeToSize:newSize];
...
- (UIImage *)tt_imageByResizeToSize:(CGSize)size contentMode:(UIViewContentMode)contentMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, self.scale);
    [self tt_drawInRect:CGRectMake(0, 0, size.width, size.height) withContentMode:contentMode clipsToBounds:NO];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return image;
}
...
- (UIImage *)tt_imageByRoundCornerRadius:(CGFloat)radius {
    return [self tt_imageByRoundCornerRadius:radius borderWidth:0 borderColor:nil];
}
...
- (UIImage *)tt_imageByRoundCornerRadius:(CGFloat)radius
    ...
    return [self tt_imageByRoundCornerRadius:radius
...
- (UIImage *)tt_imageByRotateLeft90 {
    return [self tt_imageByRotate:DegreesToRadians(90) fitSize:YES];
}
