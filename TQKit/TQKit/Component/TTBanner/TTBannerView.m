...
// 总共的item数
#define TT_TOTAL_ITEMS (self.itemCount * 10000)

#define TT_FOOTER_WIDTH 64.0
#define TT_PAGE_CONTROL_HEIGHT 32.0

@interface TTBannerView ()<UICollectionViewDelegate, UICollectionViewDataSource>


@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionViewFlowLayout *flowLayout;

@property (nonatomic, strong) TTBannerFooter *footer;
...
- (void)setCurrentIndex:(NSInteger)currentIndex animated:(BOOL)animated
{
    if (self.isCycle) {
        ...
        if(newCurrentIndex >= TT_TOTAL_ITEMS) {
            return;
        }
...
{
    TTBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:BANNER_ITEM forIndexPath:indexPath];
    if ([self.dataSource respondsToSelector:@selector(tt_bannerView:viewForItemAtIndex:)]) {
        cell.containerView = [self.dataSource tt_bannerView:self viewForItemAtIndex:indexPath.item % self.itemCount];
    }
    return cell;
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)theCollectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)theIndexPath
{
    ...
    if(kind == UICollectionElementKindSectionFooter)
    {
        ...
        self.footer = (TTBannerFooter *)footer;
        
        // 配置footer的提示语
        if ([self.dataSource respondsToSelector:@selector(tt_bannerView:titleForFooterWithState:)]) {
            self.footer.normalTitle = [self.dataSource tt_bannerView:self titleForFooterWithState:TTBannerFooterStateNormal];
            self.footer.enterTitle = [self.dataSource tt_bannerView:self titleForFooterWithState:TTBannerFooterStateEnter];
        }
        
        self.footer.bounds = CGRectMake(0, 0, TT_FOOTER_WIDTH, self.bounds.size.height);
    }
...
{
    if ([self.delegate respondsToSelector:@selector(tt_bannerView:didSelectItemAtIndex:)]) {
        [self.delegate tt_bannerView:self didSelectItemAtIndex:(indexPath.item % self.itemCount)];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    {
        // 开始出现footer
        if (footerDisplayOffset > TT_FOOTER_WIDTH) {
            if (lastOffset > 0) return;
            self.footer.state = TTBannerFooterStateEnter;
        } else {
            if (lastOffset < 0) return;
            self.footer.state = TTBannerFooterStateNormal;
        }
        lastOffset = footerDisplayOffset - TT_FOOTER_WIDTH;
    }
}
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    // 通知footer代理
    if (footerDisplayOffset > TT_FOOTER_WIDTH) {
        if ([self.delegate respondsToSelector:@selector(tt_bannerFooterDidEnter:)]) {
            [self.delegate tt_bannerFooterDidEnter:self];
        }
    }
}
...
- (void)updateSubviewsFrame
{
    ...
    self.flowLayout.footerReferenceSize = CGSizeMake(TT_FOOTER_WIDTH, self.frame.size.height);
    ...
    if (CGRectEqualToRect(self.pageControlFrame, CGRectZero)) {
        ...
        CGFloat h = TT_PAGE_CONTROL_HEIGHT;
        CGFloat x = 0;
...
- (void)fixDefaultPosition
{
    ...
    if (self.isCycle) {
        // 总item数的中间
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:(TT_TOTAL_ITEMS / 2) inSection:0]
...
- (void)didScrollItemAtIndex:(NSInteger)index
{
    ...
    if ([self.delegate respondsToSelector:@selector(tt_bannerView:didScrollToItemAtIndex:)]) {
        [self.delegate tt_bannerView:self didScrollToItemAtIndex:index];
    }
}
 ...
 *  数据源
 */
- (void)setDataSource:(id<TTBannerViewDataSource>)dataSource
{
...
- (void)setDelegate:(id<TTBannerViewDelegate>)delegate {
    _delegate = delegate;
...
- (NSInteger)itemCount
{
    if ([self.dataSource respondsToSelector:@selector(tt_numberOfItemsInBannerView:)]) {
        return [self.dataSource tt_numberOfItemsInBannerView:self];
    }
    return 0;
}
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        // 注册cell
        [_collectionView registerClass:[TTBannerCell class] forCellWithReuseIdentifier:BANNER_ITEM];
        
        // 注册 \ 配置footer
        [_collectionView registerClass:[TTBannerFooter class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:BANNER_FOOTER];
        _collectionView.contentInset = UIEdgeInsetsMake(0, 0, 0, -TT_FOOTER_WIDTH);
    }
...
- (UICollectionViewFlowLayout *)flowLayout
{
    if (!_flowLayout) {
        ...
        _flowLayout.footerReferenceSize = CGSizeMake(TT_FOOTER_WIDTH, 100);
    }
    return _flowLayout;
}
