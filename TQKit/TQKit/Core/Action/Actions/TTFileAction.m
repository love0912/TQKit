...
- (void)uploadImage2UrlString:(NSString *)urlString
    ...
    if (imageData == nil) {
        [TTProgressHUD showInfoTips:@"图片路径错误，请检查路径"];
        return;
    }
    NSString *imageType = [self typeForImageData:imageData];
    if (imageType == nil) {
        [TTProgressHUD showInfoTips:@"图片信息错误，请上传PNG,JPEG,GIF等图片"];
        return;
    }
...
- (NSURLSessionDownloadTask *)downloadFile2UrlString:(NSString *)urlString
    ...
    if (!_apiClient.isReachable) {
        [TTProgressHUD showErrorTips:@"网络未连接,请检查网络"];
        return nil;
    }
    //通过afnetworking的分类，返回downloadTask;
    NSURLSessionDownloadTask *downloadTask = [_apiClient tt_downloadTaskWithRequest:request progress:progress destination:^NSURL * _Nonnull(NSURL * _Nonnull targetPath, NSURLResponse * _Nonnull response) {
...
- (void)networkingChangedNotReachable {
    if (_downloadingTaskArray.count > 0) {
        ...
        [TTAlertUtil showAlertWithTitle:@"下载未完成，请检查网络连接" message:nil cancleButtonTitle:@"确定" OtherButtonsArray:nil showInController:nil clickAtIndex:^(NSInteger buttonIndex) {
...
- (void)showNetworkingWANDownloadTipByClick:(void(^)(NSInteger buttonIndex)) click {
    [TTAlertUtil showAlertWithTitle:@"下载未完成，是否在移动数据下载?" message:nil cancleButtonTitle:@"暂停下载" OtherButtonsArray:@[@"土豪请继续"] showInController:nil clickAtIndex:^(NSInteger buttonIndex) {
        click(buttonIndex);
    }];
}
